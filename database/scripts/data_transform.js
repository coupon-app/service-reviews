...
const generateReviews = () => {
  ...
  for (let productId = 1; productId <= numMaxProductIds; productId += 1) {
    ...
    for (let i = 0; i < productIdReviewNumber; i += 1, reviewIdCount += 1) {
      i = i >= data.length ? 0 : i;
      let review_id = i; // Auto-incrementing integer (key) (REQUIRED)
      let date_created = data[i].unixReviewTime * 1000; // Epoch time (or perhaps in some other ISO string format) (REQUIRED)
      let product_id = productId; // Integer between 1 and numMaxProductIds (initially) (REQUIRED)
      let customer = { // Embedded object representing the details of the customer who made the review (REQUIRED)
        customer_id: data[i].reviewerID, // String representing a unique customer ID (REQUIRED)
      ...
      let picture = { // Embedded object representing a customer's uploaded image (NOT REQUIRED)
        ...
        picture_upload_date: data[i].unixReviewTime, // Epoch time (or perhaps in some other ISO string format) (NOT REQUIRED)
      };
